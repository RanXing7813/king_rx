...
public class DeptController  extends BaseAction {
	...
	public static final String ReqMapping = "sys/DeptController!" ;
	...
	*/
	...
	public Object getList(Model model,  
		...
		try {
			pageUtil =  deptService.getPageAll( init(models), pageUtil);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return pageUtil;
	...
	* @throws
	...
	public Object  newDetail(Model model, PageUtil pageUtil,@ModelAttribute(value="dto") DeptDto dto, HttpServletRequest request){
			...
			String str = deptService.saveDept(dto, userDto);
	...
	* @throws
	...
	public Object  removeDetail(Model model, PageUtil pageUtil, @RequestParam(value="id", required=true)String id, HttpServletRequest request ){
		 ...
		 String str;
		try {
			str = deptService.remove(id, userDto);
			pageUtil.setMessage(str);
		} catch (Exception e) {
			e.printStackTrace();
